...
# Unreleased
 - Fix `candle restart` (and `candle kill` followed by `candle start`) sometimes showing the previous instance's last output and exit (e.g. `[Process exited with code 0]`) in `candle logs` as if they came from the new run. This happened mostly when a background process kept the old service's output open. `start` now also waits for an instance that was already marked killed to finish writing its logs, and `logs` / `wait-for-log` no longer treat a previous instance's exit as the current run's exit. Found with the new Lean models in `formal/`.
