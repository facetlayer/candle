...
# Unreleased
 - Fix `candle kill` leaving a service running when it traps or ignores `SIGTERM`, while reporting it killed. Kill still sends `SIGTERM` to the process tree first, but now waits up to 5s for it to exit and then sends `SIGKILL` to the whole tree, re-snapshotted to catch newly forked children. The escalation is reported on stderr. `start` and `restart` share this path when they replace a running instance.
