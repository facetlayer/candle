...
# Unreleased
 - Fix a service dying if it printed output that isn't valid UTF-8: the monitor stopped reading that pipe, so the service's next write killed it with SIGPIPE. Invalid bytes are now stored as `�`.
 - Invalid numeric flag values are now errors instead of being silently ignored: `--count abc` used to fall back to 100 and `--count -1` printed every line; now they fail with `Error: --count must be a number, got 'abc'` / `Error: --count must be at least 1, got -1`. The same applies to `--start-at`, `--timeout` (which must also be non-negative) and `--exit-after-ms`.
 - `wait-for-log` with no service names now labels its failure output `Logs from the latest run of each service:` instead of `... of '':`.
 - A service killed after startup by a signal Candle didn't send (a segfault, the OOM killer, `kill -9` from elsewhere) now shows `FAILED` in `ps` / `list`, and its logs end with `[Process was killed by signal <n>]`. It used to show `not running` with `[Process was stopped]`, like a deliberate `candle kill`.
 - Fix `candle restart` (and `kill` then `start`) sometimes showing the previous instance's last output and exit, e.g. `[Process exited with code 0]`, in `candle logs` as if they came from the new run. Every log row now records which run of the service wrote it, and `logs`, `wait-for-log`, `watch` and `ps` / `list` look only at the latest run, whatever order the rows were written in. As a result, `start` / `restart` no longer wait up to 2s for the previous instance to finish writing its logs, and that instance's late output is still kept rather than cut off. The database gains a `run_id` column, filled in for existing rows on first use. Found with the new Lean models in `formal/`.
 - The `--mcp` flag is removed; use `candle mcp`. In `candle --help`, `help` and `mcp` move from "Options" to a new "Other" section, `--json` is listed as a global option, `kill` shows its `stop` alias, and the `add-service`, `list-ports` and `open-browser` lines are reworded. `set-config --help` now lists the valid keys and defaults; `list-docs --help`, `get-doc --help` and `mcp --help` describe their output.
 - `candle list-docs` prints one doc per line as `<name>  <description>`, and every doc now has a description. `candle get-doc` with no name is a usage error (`get-doc requires a <name>`) instead of `Doc file not found: `.
 - Built-in docs reorganized: `getting-started` is replaced by `project-setup` (creating `.candle.json`, the service fields, and log retention), a new `mcp-usage` doc covers launching `candle mcp` and its tools, and `transient-processes` is shortened. The docs no longer mention `candle watch` or `candle stop`.
 - `candle list-ports` / `list-ports-all` (and `open-browser`, MCP `ListPorts`) now work on Linux and Windows, not just macOS. Linux reads `/proc/net/tcp` directly, so `lsof` is no longer needed there; Windows uses `netstat`. Child processes are now also discovered on Windows (via PowerShell). If the platform tool is missing, the command now fails with `Error: Could not detect listening ports: the \`lsof\` command was not found. ...` and an install hint, instead of misreporting "No open ports found".
 - `candle list-docs` / `get-doc` no longer include developer docs. `testing-strategy` moved to `docs/dev/`, which is not built into the binary's doc list.
 - `candle ps` / `candle list` (and `--json` / MCP `ListServices`) show a new status, `FAILED`, for a service whose latest start failed without an exit code: its `root` directory doesn't exist, the shell couldn't be launched, or it was killed by a signal during the startup grace period. Its `exitCode` is `null`. A start that exited non-zero still shows `EXITED (<code>)`. Stopping a service with `kill` / `restart` never shows `FAILED`, even during startup: `kill` now marks the process killed before signalling it, and the monitor logs such a stop as `Process was stopped while starting`. A missing `root` caught before launch is now also recorded in the service's logs, so `candle logs <name>` shows why. (`list-all` lists only running processes, so it never shows `EXITED` or `FAILED`.)
 - Every command that takes a service name now reports an unknown one with the same text, `No service '<name>' configured for directory: <dir>`. `list` / `ps` used to omit the directory. `clear-logs` and `watch` now reject unknown names too (`clear-logs` used to print "No logs found to clear", `watch` "Process '<name>' is not running"); `clear-logs` accepts the same names as `logs`. `list-all <name>` with no running match says `No running service named '<name>'`.
 - The MCP `GetLogs` tool now errors for a service name that isn't configured, like `candle logs`, while still accepting a name with stored logs or a process (such as a finished transient service). `KillService` errors on an unknown name, like `candle kill`.
 - After a service exits, the monitor now waits at most 500ms (was 2s) for output still in flight. The limit only matters when a background grandchild keeps the output pipes open; lines it writes after that are not captured.
 ...
 - `list --json` / `ps --json` / `list-all --json` rows now always have the same keys: a stopped service has `"pid": null` (was `0`) and `"configChanged": false` (was omitted), and every row has a new `exitCode` (the non-zero exit code of the latest run, else `null`). The MCP `ListServices` output changes the same way. An unknown name for `list` / `ps` now reads `No service '<name>' configured for directory: <dir>`.
 ...
 - Fix the monitor sometimes dropping a process's last output lines, such as a fast failure's error message. After the process exits, the monitor keeps collecting output until both pipes close, for up to 500ms, since a background child can hold them open indefinitely.
