...
export function updateProcessKilledAt(entry: UpdateProcessKilledAt): void {
    const db = getDatabase();
    db.run('update processes set killed_at = ? where command_name = ? and project_dir = ? and pid = ?',
...
export function deleteProcessEntry(entry: DeleteProcessEntry): void {
    ...
    db.run('delete from processes where command_name = ? and project_dir = ? and pid = ?',
        [entry.commandName, entry.projectDir, entry.pid]);
}

export function findProcessesByCommandNameAndProjectDir(commandName: string, projectDir: string): ProcessEntry[] {
    const db = getDatabase();
...
export function findProcessesByProjectDir(projectDir: string): ProcessEntry[] {
    const db = getDatabase();
    return db.list('select * from processes where project_dir = ?',
        [projectDir]);
}
