...
/** PID a service printed as `daemon <pid>` (the child it double-forked). */
async function daemonPidOf(service: string): Promise<number> {
  const logs = (await workspace.runCli(['logs', service])).stdoutAsString();
  return Number(/daemon (\d+)/.exec(logs)![1]);
}
...
describe('process safety', () => {
  ...
  describe('kill', () => {
    ...
    it('kills a double-forked child that was reparented away from the service', async () => {
      await workspace.runCli(['start', 'daemonizer']);
      await workspace.runCli(['wait-for-log', 'daemonizer', '--message', 'ready']);
      const daemonPid = await daemonPidOf('daemonizer');
      expect(isAlive(daemonPid)).toBe(true);

      await workspace.runCli(['kill', 'daemonizer']);

      expect(isAlive(daemonPid)).toBe(false);
    }, 20000);

    it('SIGKILLs a double-forked child that ignores SIGTERM', async () => {
      await workspace.runCli(['start', 'stubborn-daemonizer']);
      await workspace.runCli(['wait-for-log', 'stubborn-daemonizer', '--message', 'ready']);
      const daemonPid = await daemonPidOf('stubborn-daemonizer');

      const result = await workspace.runCli(['kill', 'stubborn-daemonizer']);
  ...
  describe('concurrent start', () => {
    it('leaves exactly one instance when restarted five times at once', async () => {
      await Promise.all(Array.from({ length: 5 }, () => workspace.runCli(['restart', 'slow'])));
