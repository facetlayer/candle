...
describe('CLI --project-dir', () => {
  ...
  describe('before the command', () => {
    it('accepts --project-dir ahead of the command, like git -C', async () => {
      const ps = await workspace.runCli(['--project-dir', project, 'ps']);
      expect(ps.stdoutAsString()).toContain('ticker');
    });

    it('accepts the --project-dir=<dir> form ahead of the command', async () => {
      const ps = await workspace.runCli([`--project-dir=${project}`, 'ps']);
      expect(ps.stdoutAsString()).toContain('ticker');
    });

    it('applies other leading options to the command', async () => {
      const ps = await workspace.runCli(['--json', '--project-dir', project, 'ps']);
      const rows = JSON.parse(ps.stdoutAsString());
      expect(rows.map((r: any) => r.serviceName)).toContain('ticker');
    });

    it('rejects a leading option the command does not take', async () => {
      const result = await workspace.runCli(['--bogus', 'ps'], { ignoreExitCode: true });
      expect(result.failed()).toBe(true);
      expect(result.stderrAsString()).toContain('Unknown argument: --bogus');
    });

    it('rejects a leading --project-dir on a system-wide command', async () => {
      const result = await workspace.runCli(['--project-dir', project, 'list-all'], {
        ignoreExitCode: true,
      });
      expect(result.failed()).toBe(true);
      expect(result.stderrAsString()).toContain('Unknown argument: --project-dir');
    });
  });
