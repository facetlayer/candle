...
describe('CLI Find-Orphans Command', () => {
  ...
  describe('with nothing orphaned', () => {
    ...
    it('does not report a running transient process', async () => {
      ...
      await workspace.runCli([
        ...
        'transient-proc',
        '--shell',
        sleeper('transient-proc').shell,
      ]);

      const out = (await workspace.runCli(['find-orphans'])).stdoutAsString();
      expect(out).not.toContain('transient-proc');
      expect(out).toContain('No orphaned processes found');
    });

    it('does not report a transient process after it is restarted', async () => {
      const dir = makeProject([]);
      started.push(dir);
      const shell = sleeper('restarted-transient').shell;
      await workspace.runCli([
        'start',
        '--project-dir',
        dir,
      ...
      await workspace.runCli(['restart', '--project-dir', dir, 'restarted-transient']);

      const out = (await workspace.runCli(['find-orphans'])).stdoutAsString();
      expect(out).not.toContain('restarted-transient');
    });
  });

  describe('detecting orphans', () => {
    it('reports a transient process whose project directory was deleted', async () => {
      const dir = makeProject([]);
      started.push(dir);
      await workspace.runCli([
        'start',
        ...
        'transient-gone',
        '--shell',
        sleeper('transient-gone').shell,
      ]);
      fs.rmSync(dir, { recursive: true, force: true });

      const out = (await workspace.runCli(['find-orphans'])).stdoutAsString();
      expect(out).toContain('transient-gone');
  ...
  describe('cleaning up what it finds', () => {
    it('the reported process can be killed with the suggested command', async () => {
      ...
      await workspace.runCli(['kill', '--project-dir', found.projectDir, found.serviceName]);
