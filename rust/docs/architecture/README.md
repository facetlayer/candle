...
| [start-flow.md](start-flow.md) | `start` (skips running services), `start_each`, the per-service start lock, the monitor handshake, transient vs configured services, success/failure detection | `start/{launch,start_one_service,start_command,service_lock}`, `monitor/{launch_info,run}`, `cli/monitor_mode`, `process_alive`, `process_tree` |
