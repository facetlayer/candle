...
Scope: the `start` / `check-start` command path, the log-collector sidecar, transient vs configured services, success/failure detection, and the `process_tree` / `process_alive` helpers. The Rust implementation lives under `rust/candle-core/src/`; `src/...` line references point at the original Node/TypeScript source, kept as the historical source of truth.
...
2. **Log-collector sidecar** (`candle-log-collector`) — a detached, long-lived process that actually spawns the user's shell command, pipes its stdout/stderr into the SQLite `process_output` table, owns the DB `processes` row lifecycle, optionally feeds stdin, and exits when the service exits.
...
```
candle start NAME
  → handle_start_command          (rust/candle-core/src/start/start_command.rs)
    → start_one_service           (rust/candle-core/src/start/start_one_service.rs)
        - check-start dedup
        ...
        - launch_with_log_collector (rust/candle-core/src/start/launch.rs)
            → spawn sidecar detached, write LaunchInfo JSON to its stdin, end stdin
              ┌─────────────── sidecar process ───────────────┐
              │ candle-log-collector                          │
              │   read launch info (stdin JSON or argv)        │
              │   monitor::start spawns user shell             │
...
## 3. `handle_start_command` (`rust/candle-core/src/start/start_command.rs`; mirrors `src/start-command.ts:25-63`)
...
## 4. `start_one_service` (`rust/candle-core/src/start/start_one_service.rs`; mirrors `src/start/startOneService.ts:45-194`)
...
- Create `LogIterator({ command_names: [name], project_dir })` (`rust/candle-core/src/logs/log_iterator.rs`), call `reset_to_latest_log_message()` (sets `current_log_id` to the id of the newest existing matching log, or null).
- `initial_log_position = log_iterator.copy()` — kept to fetch "recent logs" for an error message later.
- `save_process_log({ command_name, project_dir, log_type: process_start_initiated })` — inserted by the **CLI**, not the sidecar.

### 4.5 Choose collector implementation

The config's optional `logCollector` field (`'node' | 'rust'`) selects which collector binary to launch; the database path is `<state_dir>/candle.db`.

### 4.6 Launch

`launch_with_log_collector({ command_name, project_dir, shell, root, enable_stdin, database_path }, { log_collector })`. See §5.
...
## 5. `launch_with_log_collector` (`rust/candle-core/src/start/launch.rs`; mirrors `src/log-collector/launchWithLogCollector.ts`)

Selects implementation by `options.log_collector == 'rust'`; default is **node**.

- **Rust path resolution** (`get_rust_collector_path`): `<ProjectRootDir>/rust/target/release/candle-log-collector`; if missing, raises `'Rust log collector not found. Build it first: cd rust && cargo build --release'`. (`ProjectRootDir` = one level above `dist/`.)
- **Node launch**: command = the node binary; args = `[<ProjectRootDir>/dist/main-log-collector.js]`. Spawn options: `stdio = ['pipe','pipe','pipe']`, `detached = true`. Then await spawn, write `JSON.stringify(launch_info)` to stdin, close stdin.
- **Rust launch**: identical except command = rust binary, args = `[]`.
...
## 7. Sidecar: `candle-log-collector` (`rust/candle-core/src/log_collector/{mod,monitor}.rs`)

### 7.1 Reading launch info
- If no argv is passed (the production path, since launchers pass no argv) → read launch info as JSON from stdin.
- Else parse argv: `--commandName` (required), `--projectDir` (required), `--shell` (required), `--root`, `--enableStdin` (bool, default false), `--databasePath`. `projectDir` is resolved to an absolute path; `databasePath` defaults to `<state_dir>/candle.db`.
...
### 7.2 Sidecar `main` lifecycle (mirrors `main-log-collector.ts:66-153`)
...
3. `create_process_entry({ command_name, project_dir, pid, log_collector_pid: own_pid, shell, root })` — **pid = user shell pid; log_collector_pid = sidecar's own pid.**
...
### 7.3 `monitor::start` (mirrors `src/log-collector/startMonitoredService.ts`)
...
`handle_kill_command` (`rust/candle-core/src/kill/mod.rs`): with names → dedupe set, `kill_by_command_name` each; without names → kill all `find_running_processes_by_project_dir`. `quiet_failure` suppresses the "No running processes" messages.

`kill_one_running_process` (`rust/candle-core/src/kill/mod.rs`):
...
`kill_process_tree` (`rust/candle-core/src/kill/mod.rs`): errors on pid null/0; `get_process_tree(pid)`; if empty → `'process_not_found'`; iterate **reversed** (children first, root last) sending `SIGTERM`; ESRCH ignored; other errors → warn + `has_error`. Returns `'process_not_found'` if every kill found nothing, `'error'` if any non-ESRCH error, else `'success'`.

## 9. `process_tree` (`rust/candle-core/src/process_tree.rs`)
...
## 10. `process_alive` (`rust/candle-core/src/process_alive.rs`)
...
## 11. Database (`rust/candle-core/src/db/mod.rs`, `rust/candle-core/src/db/process_table.rs`)
SQLite at `<state_dir>/candle.db`. `state_dir` (`rust/candle-core/src/dirs.rs`; mirrors `src/dirs.ts:9-22`): `$CANDLE_DATABASE_DIR`, else `$XDG_STATE_HOME/candle`, else `~/.local/state/candle`. Pragmas: `journal_mode=WAL`, `busy_timeout=30000` (multi-process access). Schema (safe-upgrade migrations):
...
- stdin (`rust/candle-core/src/db/stdin_messages.rs`): `pop_stdin_message` = `select * ... order by id asc limit 1` then `delete ... where id=?`; `clear_stdin_messages` = `delete ... where command_name=? and project_dir=?`.

### `ProcessLogType` (`rust/candle-core/src/logs/log_type.rs`) — integer values are persisted, must match exactly:
`stdout=1, stderr=2, process_start_initiated=3, process_start_failed=4, process_started=5, process_exited=6`.

### Log polling (`LogIterator` + `get_process_logs`, `rust/candle-core/src/logs/{log_iterator,process_logs}.rs`)
...
2. **Detached but not unref'd**: sidecar survival relies on the CLI explicitly exiting after the watch loop; the sidecar runs in its own session. The Rust implementation spawns with a new session and does not join.
