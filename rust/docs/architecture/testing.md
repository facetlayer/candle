...
| `help.test.ts` | `cli-help` | `--help`/`help`/no-args: section headers `Process Management:`, `Port Detection:`, `Logs:`, `Configuration:`, `Documentation:`, `Troubleshooting & Maintenance:`, `Options:`; lists all commands; per-command `--help`; `help nonexistent` → `Unknown help topic`; `help <cmd>` equals `<cmd> --help`; `--enable-stdin` hidden from command help but still accepted; uses `normalizeOutput` snapshot |
...
| `restart.test.ts` | `cli-restart`, `cli-restart-reload` | `restart [name]`: `Started`; starts if not running; transient reuses DB shell; `restart <name> --shell` replaces a transient's command (and requires exactly one name); bare restart starts every configured service when nothing is running (no `No running processes` noise) and kills each service at most once; empty config → exit1 + `No services to restart` (workspace `cli-restart-empty`); unknown → `No service`; ≤1 `Killed` message; **reloads edited shell from config on restart** (marker-v1→v2) |
...
| `start.test.ts` | `cli-start`, `cli-start-empty`, `cli-start-drift`, `cli-start-stale`, `cli-start-partial-failure` | `start [names...]`/config services; multiple; all (no name); unknown → fail; `--shell` transient; `--shell --root`; `--root` without `--shell` fail; `--root` escape fail (`root`); transient shadows a stopped config service; a different `--shell` for a running service → error pointing at `candle restart <name> --shell`; same `--shell` → no-op; already running → `[Service '<name>' is already running (pid N, up T); use 'candle restart <name>' to restart it]`, same pid, no `Started`/`Killed`; mixed running/stopped names start only the stopped ones; config drift → `with an outdated command` message; **dead-PID stale row → starts anyway**; a failing service in the middle doesn't stop the rest (`1 of 3 services failed to start: broken`, others RUNNING); no config (`/tmp`) → stderr `.candle.json`; empty config → stderr `No services configured` |
...
3. Exact output strings are load-bearing:
   ...
   - Start of a running service: `[Service '<name>' is already running (pid <pid>, up <uptime>); use 'candle restart <name>' to restart it]` (tests match `already running`, and the full form by regex); drift variant contains `with an outdated command` and `use 'candle restart <name>' to apply the .candle.json changes`.
   - Multi-service start/restart with failures: each `Error: <e>` on stderr, then `<N> of <M> services failed to start: <names>` (prefixed `Failed to restart: ` for restart).
   - Errors: `No service '<name>' configured for directory: <cwd>`; `No .candle.json file found in (or above) current directory: <cwd>`; `Process '<name>' failed to start. Recent logs:` + newline + log lines; `Process '<name>' failed to start: root directory does not exist: <dir>`; `No services configured in .candle.json`; `No services to restart: none are configured in .candle.json and none are running`; `Service '<name>' is already running with a different command: $ <shell>`; `Exactly one service name is required when using --shell`; `Unrecognized command '<cmd>'`.
