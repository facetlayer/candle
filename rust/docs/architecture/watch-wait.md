...
### 1.2 `ProcessLog` row shape (`logs/process_logs.rs`)
`ProcessLog { id: i64, command_name: String, project_dir: String, content: Option<String>, log_type: i64, timestamp: i64, run_id: Option<i64> }`.

### 1.3 `ProcessLogType` enum (`logs/log_type.rs`) — exact integer values
```
stdout                  = 1
stderr                  = 2
...
## 2. The DB query: `build_log_search_query` (`logs/process_logs.rs`)
...
## 4. `LatestRunFilter` (`log_filters/latest_run_filter.rs`)
...
## 5. Counting running services

`watch` counts still-running services from the `processes` table at exit (§7.4), not from the log stream.

## 6. Output formatting (`logs/console_log.rs`)

`console_log_row(row, ConsoleLogOptions { format, prefix, enable_app_name_prefix })` dispatches on `log_type` (`content` of `None` renders as empty):
- `stdout (1)`: pretty → `prefix + content`; json → `{ stdout: content }`
- `stderr (2)`: pretty → `prefix + "[stderr] " + content`; json → `{ stderr: content }`
- `process_exited (6)` / `process_start_failed (4)`: routed through `console_log_system_message` → pretty → `prefix + "[" + content + "]"`; json → `{ message: content }`
- `process_start_initiated (3)` / `process_started (5)`: **suppressed (nothing printed).**

`console_log_system_message(format, msg, prefix)`: pretty → `prefix + "[" + msg + "]"`; json → `{ message: msg }`.

Log rows always go to **stdout**, including `stderr`-typed rows. Each call emits one line (adds `\n`).

`enable_app_name_prefix` prepends `[<command_name>] `; `watch` builds its own `prefix` instead, while `wait-for-log`'s recent-logs dump uses `enable_app_name_prefix` (§8.3).

## 7. `watch` command (`rust/src/commands/watch.rs`)

### 7.1 CLI definition (`cmd_watch` in `rust/src/main.rs`)
`watch [name...]` — positional `name` (variadic, strings). Options `--exit-after-ms <ms>` (accepted by the hand-rolled parser but not listed in `watch --help`, so effectively hidden) and `--project-dir <dir>` (must contain its own config, `require_own_config`). A non-numeric or negative `--exit-after-ms` is a fatal usage error. Unknown flags are rejected (`Unknown argument`).

### 7.2 Agent-mode disabling (`run_context.rs`)
...
`is_run_by_agent` is evaluated once and cached: agent mode iff **any** of `CLAUDECODE` / `GEMINI_CLI` / `CURSOR_AGENT` is present and non-empty. The empty string means not-set; `"0"`/`"false"` are non-empty and therefore still count.

### 7.3 `handle_watch(conn, cwd, command_names, exit_after_ms)` (`rust/src/commands/watch.rs`)

`watch` **never launches processes**.
...
- After loop: `running = count_running_services(conn, project_dir, command_names)`: the number of distinct watched services (all in the project when no names) with a live `processes` row (`find_running_processes_by_project_dir` + `filter_alive_processes`):
  - `== 1`: `[Stopped watching. Process is still running in the background.]`
...
### 8.1 CLI definition (`cmd_wait_for_log` in `rust/src/main.rs`)
`wait-for-log [name]` — positional name(s), all passed through as `command_names`. Required option `--message <string>` (missing → stderr `Error: --message <text> is required`, exit 1). Option `--timeout <number>` in seconds (fractional allowed), default `30`; a non-numeric, negative or non-finite value is a fatal usage error. Option `--project-dir <dir>`. Strict options. **Not** disabled in agent mode. Names are validated with `assert_known_service_names` (same rule as `logs`: stored logs, a process row, or a config entry), so an unknown name prints `No service '<name>' configured for directory: <dir>` to stderr and exits 1. Transient names with history are allowed.

Dispatch: resolve `project_dir`, call `handle_wait_for_log(conn, project_dir, command_names, message, timeout_ms = (timeout * 1000) as u64)`, and exit `1` if `!result.success`. So **exit code 0 on success, 1 on failure.** Timeout is converted seconds→ms here.

### 8.2 `handle_wait_for_log` (`rust/src/commands/wait_for_log.rs`)
...
Return shape: `WaitForLogResult { success: bool }`.

Algorithm:
1. `log_filter = LatestRunFilter::new(None)` (**no recency window**), seeded with `seed_latest_runs`. `LogIterator::with_limit(project_dir, command_names, Some(1000))`; `initial_logs = log_filter.filter(iterator.get_next_logs(conn, None))` (advances the cursor to newest). Because every row carries its run, this is right even when the launch itself is older than the 1000-row window.
2. Scan `initial_logs`: if any row's `content` contains `message` (substring match; a `None` content is skipped): print `Found message "<message>" in existing logs.` and return `{ success: true }`. This runs first, so a run that has already finished still satisfies the wait if it printed the message.
...
7. Poll loop (`time_started = Instant::now()`):
   - Once the start is reported, every `LIVENESS_CHECK_EVERY` polls (starting with the first): if nothing is running, fail with recent logs (`fail_not_running`). Before the start is reported the launch is still in progress, so the missing row is expected.
   - If elapsed `> timeout_ms`: print `Error: Timed out after <timeout_ms>ms and message "<message>" not found.` to stderr, call `print_recent_logs(...)`, return `{ success: false }`.
   - `raw_logs = iterator.get_next_logs(conn, None)` (limit 1000); `logs = log_filter.filter(raw_logs)`.
   - For each log: if `content` contains `message` → print `Found message "<message>" in logs.` and return `{ success: true }`. A `process_started` row sets `start_reported`. Else if `log_type` is `process_exited (6)` or `process_start_failed (4)` → print `Error: Process exited before finding message "<message>"` to stderr, call `print_recent_logs(...)`, return `{ success: false }`.
   - Sleep 200ms.

The timeout is checked at the **top** of the loop (after the liveness check) before fetching; the first check happens immediately (0 elapsed, won't trip). The exit-before-found check is per-log within a batch and is evaluated **after** the message check, so a batch where the matching line and the exit line both appear returns success if the match comes first in chronological order.

### 8.3 `print_recent_logs`
Prints on failure paths (to stdout), showing only the tail of the latest run:
...
- Header: `Last 20 lines of the latest run of '<names>':` when the tail was truncated, else `Logs from the latest run of '<names>':` (`each service` in place of `'<names>'` when no name was given).
- Each row via `console_log_row` (pretty; `enable_app_name_prefix` when there isn't exactly one name).
- Footer: `Run 'candle logs <name>' to see more.` (`candle logs` with zero or several names).

`fail_not_running` prints `Error: Service '<name>' is not running and message "<m>" was not found.` to stderr (`Services '<a, b>' are not running` for several names, `No service in this project is running` for none), then the recent logs when the service has a recorded run.
...
- `Error: Timed out after <ms>ms and message "<m>" not found.` (stderr, failure)
- `Error: Process exited before finding message "<m>"` (stderr, failure)
- `Error: Service '<name>' is not running and message "<m>" was not found.` (stderr, failure)
...
- **SQLite access** (`get_database()`): `rusqlite`, synchronous.
...
1. **Timestamp unit mismatch:** DB `timestamp` is UNIX **seconds**; wall-clock math is **ms**. The recency window divides ms by 1000 (keeping the fraction, no rounding) and compares against integer seconds.
...
3. **`after_log_id` semantics:** absent → no filter; `0` is valid and applies `id > 0`. Modeled as `Option<i64>` with `Some(0)` distinct from `None`.
...
7. **stdout vs stderr:** log rows, headers and success lines are stdout. Errors go to **stderr** with an `Error: ` prefix: the agent-mode watch error, the `watch` "is not running" usage error, and every `wait-for-log` failure line (the recent-logs dump that follows it is stdout).
...
Tests: `test/cli/watch.test.ts`, `test/cli/watch-restart.test.ts`, `test/cli/wait-for-log.test.ts`.
