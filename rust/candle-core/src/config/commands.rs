    ...
    match key {
        "logCollector" => {
            if value == "node" || value == "rust" {
                Ok(ParsedConfigValue::LogCollector(value.to_string()))
            } else {
                Err(CandleError::UsageError(format!(
                    "Invalid value for 'logCollector': expected 'node' or 'rust', got '{value}'"
                )))
            }
        }
...
fn apply_config_value(config: &mut CandleSetupConfig, parsed: &ParsedConfigValue) {
    match parsed {
        ParsedConfigValue::LogCollector(s) => {
            config.log_collector = Some(s.clone());
            config.ensure_key("logCollector");
        }
...
mod tests {
    ...
    #[test]
    fn set_config_log_collector() {
        let dir = TempDir::new();
        std::fs::write(dir.path().join(".candle.json"), "{\n  \"services\": []\n}").unwrap();
        let msg = handle_set_config("logCollector", "rust", dir.path()).unwrap();
        assert_eq!(msg, "Set 'logCollector' to 'rust' in .candle.json");
        let contents = read_to_string(&dir.path().join(".candle.json"));
        assert_eq!(
            contents,
            "{\n  \"services\": [],\n  \"logCollector\": \"rust\"\n}"
    ...
    fn set_config_unknown_key() {
        ...
        assert_eq!(
            err.to_string(),
            "Unknown config key 'bogus'. Valid keys: logCollector, logEviction.maxLogsPerService, logEviction.maxRetentionSeconds"
        );
        assert!(err.is_usage_error());
    }

    #[test]
    fn set_config_invalid_collector() {
        let dir = TempDir::new();
        std::fs::write(dir.path().join(".candle.json"), "{\n  \"services\": []\n}").unwrap();
        let err = handle_set_config("logCollector", "go", dir.path()).unwrap_err();
        assert_eq!(
            err.to_string(),
            "Invalid value for 'logCollector': expected 'node' or 'rust', got 'go'"
        );
    }
