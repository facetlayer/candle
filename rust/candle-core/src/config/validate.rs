...
const KNOWN_TOP_LEVEL_KEYS: [&str; 3] = ["services", "logEviction", "logCollector"];
...
pub fn validate_config(value: Value) -> Result<CandleSetupConfig, CandleError> {
    ...
    let log_collector = parse_log_collector(obj.get("logCollector"))?;
...
fn parse_log_collector(value: Option<&Value>) -> Result<Option<String>, CandleError> {
    let value = match value {
        None => return Ok(None),
        Some(v) => v,
    };
    ...
    Err(CandleError::ConfigFileError(format!(
        "Config file error: Invalid value for 'logCollector': expected 'node' or 'rust', got '{got}'"
    )))
}
...
mod tests {
    ...
    #[test]
    fn log_collector_enum() {
        let cfg = validate_config(json!({ "services": [], "logCollector": "rust" })).unwrap();
        assert_eq!(cfg.log_collector.as_deref(), Some("rust"));

        let err = validate_config(json!({ "services": [], "logCollector": "go" })).unwrap_err();
        assert_eq!(
            err.to_string(),
            "Config file error: Invalid value for 'logCollector': expected 'node' or 'rust', got 'go'"
        );
    }
