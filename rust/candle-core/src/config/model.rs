...
pub struct CandleSetupConfig {
    ...
    pub log_collector: Option<String>,
    /// Top-level key insertion order, used to reproduce JS object ordering on
    /// write-back. Known keys (`services` / `logEviction` / `logCollector`) and
    /// unknown keys (held in `extra`) both appear here.
    ...
    pub fn to_value(&self) -> Value {
        ...
        for key in &self.key_order {
            match key.as_str() {
                ...
                "logCollector" => {
                    if let Some(lc) = &self.log_collector {
                        map.insert("logCollector".to_string(), Value::String(lc.clone()));
                    }
                }
                other => {
