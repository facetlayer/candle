...
use crate::start::launch::launch_with_log_collector;
...
pub fn start_one_service(conn: &Connection, opts: RunOptions) -> Result<StartResult, CandleError> {
    ...
    // 5. Launch the (always-Rust) log-collector sidecar.
    let info = LogCollectorLaunchInfo {
    ...
    launch_with_log_collector(&info)
        .map_err(|e| CandleError::Generic(format!("Failed to launch log collector: {e}")))?;
